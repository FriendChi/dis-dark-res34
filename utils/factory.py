...
def get_optimizer(net,cfg):
    #用filter函数来筛选出模型中需要梯度更新的参数，返回一个迭代器
...
def get_scheduler(optimizer, cfg, iters_per_epoch):
    #判断cfg中的调度器类型是否是'multi'
    if cfg.scheduler == 'multi':
        #用MultiStepLR类来创建一个多步长学习率调度器
#传入优化器，学习率衰减的步长，学习率衰减的比例，每个轮数的迭代次数，是否使用预热期和预热期的迭代次数等参数
    ...
    #用CosineAnnealingLR类来创建一个余弦退火学习率调度器
